...
export class CardBattleScene extends BaseScene {
  ...
  private setupLayout(): void {
    ...
    this.createBattlefieldArea(currentY, battlefieldHeight);
  ...
  private createBattlefieldArea(y: number, height: number) {
    // Create battlefield background and character containers
    ...
    battlefieldBg.roundRect(0, 0, this.gameWidth, height, 10)
      .fill(Colors.UI_BACKGROUND)
      .stroke({ width: 2, color: Colors.UI_BORDER });
    
    this.battlefieldContainer.addChild(battlefieldBg);
    this.battlefieldContainer.y = y;
    
    // Position character containers
    // const characterAreaHeight = height / 2 - 10; // Not used currently
    
    // Opponent characters (top half)
    this.player2CharactersContainer.y = 5;
    
    // Action log in center
    this.createActionLogInCenter(y + height / 2 - 20, 40);
    
    // Player characters (bottom half)  
    this.player1CharactersContainer.y = height / 2 + 25;
  ...
  // UI update and management methods
  private updateTurnIndicator(): void {
    // Update turn indicator based on current phase and player
    if (this.turnIndicatorContainer && this.battleState) {
      this.turnIndicatorContainer.removeChildren();
      
      const indicatorText = new Text({
        text: `Player ${this.battleState.current_player} - ${this.currentPhase}`,
        style: {
          fontFamily: 'Kalam',
          fontSize: 14,
          fill: Colors.TEXT_PRIMARY,
      ...
      indicatorText.x = this.gameWidth / 2;
      indicatorText.y = 22;
      
      this.turnIndicatorContainer.addChild(indicatorText);
  ...
  private updateEnergyIndicator(): void {
    ...
    if (player1 && this.energyContainer.children.length > 1) {
      const energyText = this.energyContainer.children[1] as Text;
      energyText.text = `Energy: ${player1.deck.current_energy || 0}`;
    }
    
    // Update opponent energy
    const player2 = this.battleState.players.find(p => p.team === 2);
    if (player2 && this.opponentEnergyContainer.children.length > 1) {
      const energyText = this.opponentEnergyContainer.children[1] as Text;
      energyText.text = `Energy: ${player2.deck.current_energy || 0}`;
    }
  }
  ...
  private updateDeckRemaining(): void {
    ...
    const player1 = this.battleState.players.find(p => p.team === 1);
    if (player1 && this.deckRemainingContainer.children.length > 2) {
      const deckCountText = this.deckRemainingContainer.children[2] as Text;
    ...
    const player2 = this.battleState.players.find(p => p.team === 2);
    if (player2 && this.opponentDeckRemainingContainer.children.length > 2) {
      const deckCountText = this.opponentDeckRemainingContainer.children[2] as Text;
      const deckCount = player2.deck.deck_cards?.length || 0;
      deckCountText.text = deckCount.toString();
    }
  }
  ...
  private updateOpponentHandCards(): void {
    ...
    // Clear existing opponent hand cards
    this.opponentHandContainer.removeChildren();
    
    // Create hand background (already handled in createHandAreaUI)
    const handBg = new Graphics();
    handBg.roundRect(0, 0, this.gameWidth, this.opponentHandContainer.height || 80, 10)
      .fill(Colors.UI_BACKGROUND)
      .stroke({ width: 2, color: Colors.UI_BORDER });
    this.opponentHandContainer.addChild(handBg);
    
    const player2 = this.battleState.players.find(p => p.team === 2);
    if (!player2 || !player2.deck.hand_cards) return;
    
    const handCount = player2.deck.hand_cards.length;
    if (handCount === 0) return;
    
    // Show card backs for opponent
    const cardWidth = 40;
    const cardSpacing = 5;
    const totalWidth = (cardWidth * handCount) + (cardSpacing * Math.max(0, handCount - 1));
    const startX = (this.gameWidth - totalWidth) / 2;
    
    for (let i = 0; i < handCount; i++) {
      const cardBack = this.createCardBack(cardWidth, cardWidth * 1.4);
      cardBack.x = startX + (i * (cardWidth + cardSpacing));
      cardBack.y = 10;
      this.opponentHandContainer.addChild(cardBack);
    }
  }
  ...
  private createPlayerCharacters(player: CardBattlePlayerState, container: Container, isOpponent: boolean): void {
    const characters = player.characters.slice(0, 3); // Max 3 characters
    const cardWidth = this.CHARACTER_CARD_WIDTH;
    const cardHeight = this.CHARACTER_CARD_HEIGHT;
    const spacing = 15;
    ...
    characters.forEach((character, index) => {
      ...
      const characterCard = this.createHeroCard(character, cardWidth, cardHeight);
      characterCard.x = x;
      characterCard.y = y;
      ...
      // Make interactive if not opponent
      if (!isOpponent) {
  ...
  private makeCharacterCardInteractive(card: Container, character: CardBattleCharacter): void {
    ...
    (card as unknown as { character: CardBattleCharacter }).character = character;
  ...
  private updateCharacterStates(): void {
    ...
    // Update all character cards with current states
    this.battleState.players.forEach(player => {
  ...
  private updateCharacterCard(card: Container, character: CardBattleCharacter): void {
    // Find and update HP text
    const hpText = this.findTextInContainer(card, (text: Text) => 
      text.text.includes('HP:') || text.text.includes('/'));
    if (hpText) {
      hpText.text = `${character.hp}/${character.max_hp || character.hp}`;
    }
    
    // Find and update ATK text
    const atkText = this.findTextInContainer(card, (text: Text) => 
      text.text.includes('ATK:'));
    if (atkText) {
      atkText.text = `ATK: ${character.atk}`;
    }
    
    // Find and update DEF text
    const defText = this.findTextInContainer(card, (text: Text) => 
      text.text.includes('DEF:'));
    if (defText) {
  ...
  private createHandCard(card: Card, x: number, y: number, cardWidth: number = this.HAND_CARD_WIDTH): Container {
    ...
    (cardContainer as unknown as { card: Card }).card = card;
  ...
  private onCardDragEnd(event: FederatedPointerEvent): void {
    ...
    const card = (this.dragTarget as unknown as { card: Card }).card;
      ...
      if (response.success && response.data) {
        ...
        if (this.battleState) {
          ...
          if (currentPlayer && currentPlayer.deck.hand_cards) {
            currentPlayer.deck.hand_cards = currentPlayer.deck.hand_cards.filter((c: unknown) => (c as { card?: Card }).card?.id !== card.id);
          }
        }
      ...
      if (response.success && response.data) {
        ...
        if (this.battleState) {
          ...
          if (logs.length > 0 && logs[0].targets) {
            logs[0].targets.forEach((target: unknown) => {
              const typedTarget = target as { team: number; id: string; after: unknown };
              const player = this.battleState!.players.find(p => p.team === typedTarget.team);
              if (player) {
                const character = player.characters.find(c => c.id === typedTarget.id);
                if (character) {
                  // Update character with after state
                  Object.assign(character, typedTarget.after);
                }
              }
            });
          }
