...
export class CardBattleScene extends BaseScene {
  ...
  private setupLayout(): void {
    ...
    // Player energy/deck/discard with enhanced discard zone
  ...
  private createEnergyDeckDiscardUI(
    ...
    // Position discard container on the right with enhanced styling for player
    containers.discard.x = startX + (elementWidth + spacing) * 2;
    containers.discard.y = yPosition;
    
    // Create enhanced discard background for player discard zone
    const discardBg = new Graphics();
    if (isPlayerDiscard) {
      // Enhanced player discard zone with glow effect
      ...
      glowBg.roundRect(-2, -2, elementWidth + 4, elementHeight + 4, 10)
        .fill({ color: Colors.DECORATION_MAGIC, alpha: 0.3 });
      containers.discard.addChild(glowBg);
    } else {
      discardBg.roundRect(0, 0, elementWidth, elementHeight, 8)
        .fill(Colors.CARD_DISCARD)
        .stroke({ width: 2, color: Colors.UI_BORDER });
    ...
    const discardLabel = new Text({
      text: isPlayerDiscard ? 'DROP ZONE' : 'DISCARD',
      style: {
        fontFamily: 'Kalam',
        fontSize: isPlayerDiscard ? 9 : 10,
        fill: Colors.TEXT_PRIMARY,
        align: 'center'
      }
    });
    ...
    // Make the player discard zone interactive for drag and drop
    if (isPlayerDiscard) {
      containers.discard.interactive = true;
      containers.discard.cursor = 'pointer';
      
      // Add hover effects for better UX
      containers.discard.on('pointerover', () => {
        const bg = containers.discard.children.find(child => child instanceof Graphics) as Graphics;
        if (bg) {
          bg.clear()
            .roundRect(0, 0, elementWidth, elementHeight, 8)
            .fill({ color: Colors.CARD_DISCARD, alpha: 0.8 })
            .stroke({ width: 3, color: Colors.SUCCESS });
        }
      });
      
      containers.discard.on('pointerout', () => {
        const bg = containers.discard.children.find(child => child instanceof Graphics) as Graphics;
        if (bg) {
          bg.clear()
            .roundRect(0, 0, elementWidth, elementHeight, 8)
        ...
        if (logs.length > 0 && logs[0].after_state) {
          ...
          if (afterState.characters && this.battleState) {
            this.battleState.players.forEach(player => {
              const teamCharacters = afterState.characters!.filter((c: CardBattleCharacter) => 
                c.team === player.team
              );
  ...
  private makeCharacterCardInteractive(card: Container, character: CardBattleCharacter): void {
    ...
    (card as Container & { character: CardBattleCharacter }).character = character;
  ...
  private createHandCard(card: Card, x: number, y: number, cardWidth: number = this.HAND_CARD_WIDTH): Container {
    ...
    // Make draggable
    this.makeHandCardDraggable(cardContainer);
    
    return cardContainer;
  }

  private makeHandCardDraggable(cardContainer: Container): void {
    cardContainer.interactive = true;
    cardContainer.cursor = 'grab';
    
    cardContainer.on('pointerdown', (event: FederatedPointerEvent) => {
      this.onCardDragStart(event, cardContainer);
    });
  ...
  private onCardDragStart(event: FederatedPointerEvent, cardContainer: Container): void {
  ...
  private getDropTarget(globalX: number, globalY: number): string | null {
    ...
    // Check if dropped on discard pile (enhanced for player discard zone)
  ...
  private async handleCardDrop(card: Card, dropTarget: string): Promise<void> {
    ...
    try {
      if (dropTarget === 'discard') {
        // Discard card for energy with enhanced animation
      ...
      if (response.success && response.data) {
        ...
        if (this.battleState) {
          ...
          if (logs.length > 0 && logs[0].targets) {
            logs[0].targets.forEach((target: { team: number; id: string; after: Partial<CardBattleCharacter> }) => {
  ...
  private animateCardToDiscard(): void {
    ...
    // Enhanced animation for discarding to the special discard zone
    gsap.to(this.dragTarget, {
      x: this.discardPileContainer.x + this.CARD_WIDTH / 2,
      y: this.discardPileContainer.y + this.CARD_HEIGHT / 2,
      scale: 0.8,
      rotation: Math.PI / 4, // Add slight rotation for visual flair
      duration: 0.6,
      ease: 'power2.out',
      onComplete: () => {
        // Add a small sparkle effect at the discard zone
        this.createDiscardEffect();
  ...
  private createDiscardEffect(): void {
    // Create a small sparkle effect at the discard zone to indicate successful discard
    const effectContainer = new Container();
    const sparkle = new Graphics();
    sparkle.star(0, 0, 4, 8, 12)
      .fill({ color: Colors.SUCCESS, alpha: 0.8 });
    
    effectContainer.addChild(sparkle);
    effectContainer.x = this.discardPileContainer.x + this.discardPileContainer.width / 2;
    effectContainer.y = this.discardPileContainer.y + this.discardPileContainer.height / 2;
