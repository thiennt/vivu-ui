...
export class BattleLogZone extends Container {
  ...
  resize(width: number, height: number): void {
    ...
    this.phaseText.style = {
      ...
      fontWeight: 'bold',
      fill: hexToPixi(Colors.WHITE),
      stroke: { color: hexToPixi(Colors.BLACK), width: 2 },
      align: 'center',
      dropShadow: {
        color: hexToPixi(Colors.BLACK),
        blur: 2,
  ...
  private drawBackground(): void {
    ...
    // Golden border (main)
    this.logBg.roundRect(0, 0, width, height, 8)
      .stroke({ width: 3, color: hexToPixi(Colors.GOLD), alpha: 0.8 });
    ...
    this.drawParchmentCorners(this.logBg, 0, 0, width, height, hexToPixi(Colors.GOLD_BRIGHT));

    // Top and bottom golden accent bars
    this.logBg.roundRect(12, 8, width - 24, 3, 1.5)
      .fill({ color: hexToPixi(Colors.GOLD_BRIGHT), alpha: 0.6 })
  ...
  showNotification(message: string, color?: number | string, duration: number = 2000): void {
    ...
    this.notificationBg.roundRect(-bgWidth / 2 + 2, -bgHeight / 2 + 2, bgWidth - 4, bgHeight - 4, 4)
      .fill({ color: hexToPixi(Colors.BROWN_DARKEST), alpha: 0.6 });
